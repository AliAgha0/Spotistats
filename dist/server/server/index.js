...
app.get("/api/me", withSpotify(async (session) => spotifyGet(session, "/me")));
app.get("/api/top/artists", withSpotify(async (session, request) => {
    ...
    return spotifyGet(session, `/me/top/artists?${query.toString()}`);
}));
app.get("/api/top/tracks", withSpotify(async (session, request) => {
    const timeRange = sanitizeTimeRange(request.query.time_range);
    const query = new URLSearchParams({ time_range: timeRange, limit: "24" });
    return spotifyGet(session, `/me/top/tracks?${query.toString()}`);
}));
app.get("/api/recent", withSpotify(async (session) => spotifyGet(session, "/me/player/recently-played?limit=50")));
...
function redirectToClient(response, params) {
...
function readSpotifyError(payload) {
